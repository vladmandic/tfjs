...
async function build() {
  ...
  if (!fs.existsSync('.tfjs-version')) {
    ...
    // @ts-ignore
    meta = await esbuild.build({ ...buildOptions.global, ...buildOptions.browser, entryPoints: ['src/tfjs-data/src/index.ts'], outfile: 'dist/tfjs-data.esm.js' });
    data = await stats(meta);
    log.state('Build Data:', { version, output: data.outputFiles, files: data.imports, inputBytes: data.importBytes, outputBytes: data.outputBytes });
    // @ts-ignore
    meta = await esbuild.build({ ...buildOptions.global, ...buildOptions.browser, entryPoints: ['src/tfjs-layers/src/index.ts'], outfile: 'dist/tfjs-layers.esm.js' });
    data = await stats(meta);
    log.state('Build Layers:', { version, output: data.outputFiles, files: data.imports, inputBytes: data.importBytes, outputBytes: data.outputBytes });
    // @ts-ignore
    meta = await esbuild.build({ ...buildOptions.global, ...buildOptions.browser, entryPoints: ['src/tfjs-converter/src/index.ts'], outfile: 'dist/tfjs-converter.esm.js' });
    data = await stats(meta);
    log.state('Build Converter:', { version, output: data.outputFiles, files: data.imports, inputBytes: data.importBytes, outputBytes: data.outputBytes });
    // @ts-ignore
